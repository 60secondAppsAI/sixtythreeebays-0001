        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalRefunds = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRefunds">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Refund</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="RefundId" type="text" placeholder="Enter RefundId" v-model="refundToAdd.refundId"></base-input>
  <base-input label="RefundAmount" type="text" placeholder="Enter RefundAmount" v-model="refundToAdd.refundAmount"></base-input>
  <base-input label="RefundDate" type="text" placeholder="Enter RefundDate" v-model="refundToAdd.refundDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="refunds" :row-key="record => record.RefundId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <RefundPictureView :refunds="refunds" />
          </div>
...
import RefundPictureView from './RefundPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'refundId',
          ...
          sorter: true
          //sorter: (a, b) => a.refundId - b.refundId,
          //sorter: (a, b) => a.refundId.localeCompare(b.refundId),
        },
        {
          title: 'Refund Amount',
		...
		dataIndex: 'refundDate',
          visible: true,
          scopedSlots: { customRender: 'refundDate' },
          sorter: true
      ...
      sortBy: 'refundId',           // Column to sort by
  ...
  methods: {
    ...
    async renderRefundsTable() {
      ...
      let refundsTableData = [];
      for (let i = 0; i < this.refunds.length; i++) {
        refundsTableData.push({
          id: i,
          refundId: this.refunds[i].refundId,
          year: this.refunds[i].year,
          date: this.refunds[i].date,
          competitionId: this.refunds[i].competitionId,
          refundId: this.refunds[i].refundId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-refunds',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRefundsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalRefunds = false;

      const currentDate = new Date().getTime();
      this.refundToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.refundToAdd);
      console.log(jsonData);
      
      const res = await RefundService.addRefund(jsonData);
